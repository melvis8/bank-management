...
const seed = async () => {
    try {
        ...
        const users = [
            ['admin', 'Admin', 'User', 'admin@bms.com', pass, '+237600000000'],
            ['jdoe', 'John', 'Doe', 'john.doe@email.com', pass, '+237699000111'],
            ['asmith', 'Alice', 'Smith', 'alice.smith@email.com', pass, '+237677000222']
        ];

        const userIds = [];
        for (const u of users) {
            const res = await pool.query(
                'INSERT INTO users (user_id, first_name, last_name, email, password_hash, phone) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (user_id) DO UPDATE SET email = EXCLUDED.email RETURNING id',
                u
            );
